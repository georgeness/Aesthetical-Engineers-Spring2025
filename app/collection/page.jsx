...
const PaintingLibrary = () => {
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col items-center py-4 transition-colors duration-300 ${theme.bg} ${theme.text}`}>
      ...
      {/* Wall View */}
      {viewMode === "wall" && filteredPaintings.length > 0 && (
            ...
            >
              <div className="relative group">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 p-2 rounded-full bg-white bg-opacity-80 hover:bg-opacity-100 text-red-500 transition-all duration-200 shadow-md transform hover:scale-110"
                >
                  {isFavorite(painting.id) ? (
                    <HeartIconSolid className="w-5 h-5" />
                  ) : (
